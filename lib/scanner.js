...
async function fullScan(targetUrl) {
  ...
  // Headless rendered-view pass (optional, env-gated). Only render pages that look like an
  // unrendered SPA shell — that's exactly where JavaScript changes the answer, and it bounds cost.
  const _rawWc = (html.seo && html.seo.wordCount) || 0;
  if (render.renderEnabled() && html && html.rendering && html.rendering.rootShell && _rawWc < 250) {
    const r = await render.renderPage(url);
    if (r && r.html) {
      const rendered = extractHtmlSignals(r.html, url, { ttfb: html.ttfb, totalLoad: html.totalLoad });
      const diff = computeRenderDiff(html, rendered);
      const rawSnapshot = {
        h1: (html.seo.h1 && html.seo.h1.count) || 0,
        wordCount: _rawWc,
        metaDescription: !!(html.seo.metaDescription && html.seo.metaDescription.exists),
        jsonLd: !!(html.aeo.jsonLd && html.aeo.jsonLd.exists),
        analyticsAny: !!(html.analytics && html.analytics.hasAny),
      };
      rendered.rendering = { ...(rendered.rendering || {}), rendered: true, raw: rawSnapshot, diff };
      html = rendered; // rendered values become authoritative for scoring + findings
    } else if (html.rendering) {
      html.rendering.rendered = false; // render attempted but unavailable → keep raw + low-confidence guard
    }
  }

  // Surface the AI-crawler policy on the AEO object so the report and findings can use it.
  if (html && html.aeo) html.aeo.aiCrawlers = (robots && robots.aiCrawlers) || { checked: false, blocked: [], blockedMajor: [], any: false };
  // Best-effort broken-link sample (conservative — only definitive 4xx/5xx count as broken).
  if (html && Array.isArray(html.internalLinkSample) && html.internalLinkSample.length) {
    html.linkCheck = await checkInternalLinks(html.internalLinkSample);
  }
  ...
  const wins = generateWins({ html, ssl, headers, robots, sitemap, observatory, pagespeed, llmsTxt, securityTxt });
...
async function fetchAndParseHtml(url) {
  ...
  const body = await resp.text();
  const totalLoad = Date.now() - startTime;
  return extractHtmlSignals(body, url, { ttfb, totalLoad });
}

// Extract every HTML signal from a document body. Works identically on raw server HTML OR a
// headless-rendered DOM snapshot, so one code path powers both the raw fetch and the rendered pass.
function extractHtmlSignals(body, url, meta = {}) {
  const ttfb = meta.ttfb || 0;
  const totalLoad = meta.totalLoad || 0;
  ...
  const detectors = {
    ...
    gtm: /googletagmanager\.com\/gtm|gtm\.js\?id=|gtm-[a-z0-9]{4,}|datalayer\s*(=|\.push|\[|\|\|)/.test(scriptText),
    metaPixel: /connect\.facebook\.net|fbq\s*\(/.test(scriptText),
    linkedin: /snap\.licdn\.com|_linkedin_partner_id/.test(scriptText),
    googleAds: /googleadservices|google_conversion|aw-\d{6,}/.test(scriptText),
    tiktok: /analytics\.tiktok\.com|ttq\.(load|track|page)/.test(scriptText),
    pinterest: /s\.pinimg\.com\/ct|pintrk\(/.test(scriptText),
    snapchat: /sc-static\.net\/scevent|snaptr\(/.test(scriptText),
    twitterads: /static\.ads-twitter\.com|[^a-z]twq\(/.test(scriptText),
    hubspot: /js\.hs-scripts\.com|js\.hs-analytics\.net|js\.hsforms|[^a-z]_hsq/.test(scriptText),
    adobe: /assets\.adobedtm\.com|\.omtrdc\.net|sc\.omtrdc|2o7\.net/.test(scriptText),
    matomo: /matomo\.js|piwik\.js|[^a-z]_paq/.test(scriptText),
    hotjar: /static\.hotjar|hotjar\.com/.test(scriptText),
    clarity: /clarity\.ms/.test(scriptText),
    fullstory: /fullstory\.com|edge\.fullstory|[^a-z]_fs_/.test(scriptText),
    mouseflow: /mouseflow\.com|[^a-z]_mfq/.test(scriptText),
    crazyegg: /crazyegg\.com|cetrk\.com/.test(scriptText),
    luckyorange: /luckyorange\.com|cdn\.lo\.js/.test(scriptText),
    mixpanel: /mixpanel/.test(scriptText),
    amplitude: /amplitude\.com|cdn\.amplitude/.test(scriptText),
    heap: /heap\.io|heapanalytics|heap\.load/.test(scriptText),
    pendo: /pendo\.io|pendo\.initialize/.test(scriptText),
    posthog: /posthog\.com|posthog\.init/.test(scriptText),
    ...
    cloudflare: /static\.cloudflareinsights\.com/.test(scriptText),
    vercel: /\/_vercel\/insights|va\.vercel-scripts\.com/.test(scriptText),
    quantcast: /quantserve\.com|quantcast\.com/.test(scriptText),
  };
  const analyticsTools = Object.keys(detectors).filter((k) => detectors[k]);
  const hasTagManager = detectors.gtm;
  const hasAdPixel = detectors.metaPixel || detectors.linkedin || detectors.googleAds || detectors.tiktok || detectors.pinterest || detectors.snapchat || detectors.twitterads;
  const hasHeatmap = detectors.hotjar || detectors.clarity || detectors.fullstory || detectors.mouseflow || detectors.crazyegg || detectors.luckyorange || detectors.mixpanel || detectors.amplitude || detectors.heap || detectors.pendo || detectors.posthog;
  ...
  // ── Client-rendering (SPA shell) detection — guards against confidently failing JS-rendered sites ──
  const rootShell = $('#root, #app, #__next, #__nuxt, [data-reactroot], [ng-version], [data-server-rendered]').length > 0;
  const likelyClientRendered = rootShell && wordCount < 100 && jsFiles >= 2;

  // ── Mixed content: insecure (http) subresources loaded by an https page ──
  let mixedContent = [];
  if (url.startsWith('https')) {
    $('script[src], img[src], iframe[src], source[src], video[src], audio[src], link[rel="stylesheet"][href]').each((_, el) => {
  ...
  // ── Internal-link sample for a conservative broken-link check (up to 10 same-origin links) ──
  let _origin = '';
  try { _origin = new URL(url).origin; } catch {}
  const _seen = new Set();
  const internalLinkSample = [];
  $('a[href]').each((_, el) => {
    if (internalLinkSample.length >= 10) return;
    const href = $(el).attr('href') || '';
    if (!href || href.startsWith('#') || /^(mailto:|tel:|javascript:)/i.test(href)) return;
    try {
      const abs = new URL(href, url).href.split('#')[0];
      if (_origin && abs.startsWith(_origin) && !_seen.has(abs) && abs !== url && abs !== url + '/') { _seen.add(abs); internalLinkSample.push(abs); }
    } catch {}
  });

  return {
    rendering: { likelyClientRendered, rootShell, bodyWordCount: wordCount },
    mixedContent: { count: mixedContent.length, examples: mixedContent.slice(0, 5) },
    internalLinkSample,
    ttfb,
    totalLoad,
    pageSize: body.length,
...
async function checkPageSpeed(url) {
  try {
    ...
    // CrUX field data — real-user metrics from Chrome (more credible than lab numbers).
    const le = (data.loadingExperience && data.loadingExperience.metrics) ? data.loadingExperience
             : (data.originLoadingExperience && data.originLoadingExperience.metrics) ? data.originLoadingExperience : null;
    let fieldData = null;
    if (le && le.metrics) {
      const m = (k) => le.metrics[k] ? { p: le.metrics[k].percentile, cat: le.metrics[k].category } : null;
      fieldData = {
        source: (data.loadingExperience && data.loadingExperience.metrics) ? 'page' : 'origin',
        overall: le.overall_category || null,
        lcp: m('LARGEST_CONTENTFUL_PAINT_MS'),
        inp: m('INTERACTION_TO_NEXT_PAINT') || m('EXPERIMENTAL_INTERACTION_TO_NEXT_PAINT'),
        cls: m('CUMULATIVE_LAYOUT_SHIFT_SCORE'),
        fcp: m('FIRST_CONTENTFUL_PAINT_MS'),
      };
    }

    return { scores, metrics, failures, fieldData };
  } catch (err) {
...
// AI crawlers worth checking for in robots.txt. Blocking these makes a site invisible to AI answers.
const AI_BOTS = [
  'GPTBot', 'OAI-SearchBot', 'ChatGPT-User', 'ClaudeBot', 'Claude-Web', 'anthropic-ai',
  'PerplexityBot', 'Perplexity-User', 'Google-Extended', 'CCBot', 'Bytespider',
  'Amazonbot', 'Applebot-Extended', 'meta-externalagent', 'cohere-ai', 'Diffbot', 'YouBot',
];
const MAJOR_AI_BOTS = ['GPTBot', 'OAI-SearchBot', 'ChatGPT-User', 'ClaudeBot', 'anthropic-ai', 'PerplexityBot', 'Google-Extended', 'CCBot'];

// Parse robots.txt into user-agent groups and report which AI crawlers are fully blocked (Disallow: /).
function parseAiCrawlers(robotsText) {
  const lines = String(robotsText || '').split(/\r?\n/).map((l) => l.replace(/#.*/, '').trim()).filter(Boolean);
  const groups = []; let cur = null;
  for (const line of lines) {
    const ua = line.match(/^user-agent:\s*(.+)$/i);
    if (ua) {
      if (cur && cur.justUA) cur.agents.push(ua[1].trim());
      else { cur = { agents: [ua[1].trim()], rules: [], justUA: true }; groups.push(cur); }
      continue;
    }
    const rule = line.match(/^(disallow|allow):\s*(.*)$/i);
    if (rule && cur) { cur.justUA = false; cur.rules.push({ allow: rule[1].toLowerCase() === 'allow', path: rule[2].trim() }); }
  }
  const fullyBlocks = (g) => g.rules.some((r) => !r.allow && r.path === '/');
  const blocked = [];
  for (const bot of AI_BOTS) {
    const g = groups.find((grp) => grp.agents.some((a) => a.toLowerCase() === bot.toLowerCase()));
    if (g && fullyBlocks(g)) blocked.push(bot);
  }
  const starGroup = groups.find((g) => g.agents.includes('*'));
  const blocksAllCrawlers = !!(starGroup && fullyBlocks(starGroup));
  return {
    checked: true,
    blocked,
    blockedMajor: blocked.filter((b) => MAJOR_AI_BOTS.includes(b)),
    blocksAllCrawlers,
    any: blocked.length > 0 || blocksAllCrawlers,
  };
}

async function checkRobotsTxt(url) {
  const empty = { checked: false, blocked: [], blockedMajor: [], blocksAllCrawlers: false, any: false };
  try {
    const origin = new URL(url).origin;
    const resp = await fetch(`${origin}/robots.txt`, { timeout: 8000 });
    if (resp.status === 200) {
      const body = await resp.text();
      return { exists: true, content: body.substring(0, 2000), size: body.length, aiCrawlers: parseAiCrawlers(body) };
    }
    return { exists: false, aiCrawlers: empty };
  } catch { return { exists: false, aiCrawlers: empty }; }
}

// Conservative broken-link sampler: HEAD a handful of same-origin links; only definitive 4xx/5xx
// count as broken. Timeouts and network errors are treated as unknown, never as broken.
async function checkInternalLinks(urls) {
  const list = (urls || []).slice(0, 10);
  if (!list.length) return { checked: false, total: 0, broken: [] };
  const results = await Promise.allSettled(list.map(async (u) => {
    const opt = (m) => ({ method: m, timeout: 5000, redirect: 'follow', headers: { 'User-Agent': 'WinTech-Site-Scanner/1.0' } });
    try {
      let resp = await fetch(u, opt('HEAD'));
      if (resp.status === 405 || resp.status === 501) resp = await fetch(u, opt('GET'));
      return { url: u, status: resp.status };
    } catch { return { url: u, status: 0 }; }
  }));
  const items = results.map((r) => r.value).filter(Boolean);
  const broken = items.filter((x) => x.status >= 400).map((x) => ({ url: x.url, status: x.status }));
  return { checked: true, total: list.length, broken };
}
...
// Compare raw vs rendered signals: which AI-critical signals appear ONLY after JavaScript runs.
function computeRenderDiff(raw, rendered) {
  const onlyAfterRender = [];
  const rawH1 = (raw.seo && raw.seo.h1 && raw.seo.h1.count) || 0;
  const renH1 = (rendered.seo && rendered.seo.h1 && rendered.seo.h1.count) || 0;
  const checks = [
    ['an H1 heading', rawH1 < 1, renH1 >= 1],
    ['the meta description', !(raw.seo && raw.seo.metaDescription && raw.seo.metaDescription.exists), !!(rendered.seo && rendered.seo.metaDescription && rendered.seo.metaDescription.exists)],
    ['structured data (schema)', !(raw.aeo && raw.aeo.jsonLd && raw.aeo.jsonLd.exists), !!(rendered.aeo && rendered.aeo.jsonLd && rendered.aeo.jsonLd.exists)],
    ['the main body content', ((raw.seo && raw.seo.wordCount) || 0) < 100, ((rendered.seo && rendered.seo.wordCount) || 0) >= 100],
    ['analytics / measurement', !(raw.analytics && raw.analytics.hasAny), !!(rendered.analytics && rendered.analytics.hasAny)],
  ];
  for (const [name, missingRaw, presentRendered] of checks) if (missingRaw && presentRendered) onlyAfterRender.push(name);
  return {
    onlyAfterRender,
    wordCountRaw: (raw.seo && raw.seo.wordCount) || 0,
    wordCountRendered: (rendered.seo && rendered.seo.wordCount) || 0,
  };
}

// Estimate how much work a fix is, so the report can triage by effort, not just severity.
//   Config  = a setting/template toggle (minutes)
//   Content = copy or written content (hours)
//   Dev     = code / engineering work (days)
function effortFor(f) {
  const s = ((f.issue || '') + ' ' + (f.cat || '')).toLowerCase();
  if (/header|security\.txt|https|charset|doctype|hreflang|robots|sitemap|canonical|viewport|lazy|favicon|open graph|twitter card|mixed content|broken link|ai crawler|blocking ai|disallow|consent banner/.test(s)) return 'Config';
  if (/schema|json-ld|structured data|lead capture|form field|missing label|landmark|aria|analytics|pixel|render-blocking|ttfb|pagespeed|lighthouse|javascript|client-?render/.test(s)) return 'Dev';
  return 'Content';
}
...
function generateFindings(data) {
  ...
  const lhPerf = pagespeed?.scores?.performance;
  if (typeof lhPerf === 'number' && lhPerf < 50) findings.push({ cat: 'Performance', issue: `Low PageSpeed score (${lhPerf}/100)`, impact: 'High', desc: 'Google Lighthouse rates this page below 50 on mobile. Slow pages lose conversions and rankings — Google uses speed as a ranking signal.' });
  else if (typeof lhPerf === 'number' && lhPerf < 80) findings.push({ cat: 'Performance', issue: `Middling PageSpeed score (${lhPerf}/100)`, impact: 'Medium', desc: 'Lighthouse mobile performance is under 80 — there is measurable speed left on the table.' });
  if (html.ttfb && html.ttfb > 800) findings.push({ cat: 'Performance', issue: `Slow server response (${html.ttfb}ms TTFB)`, impact: 'Medium', desc: 'Time to first byte over 800ms. A CDN or server-side caching is the usual fix; Google factors response time into rankings.' });
  if (html.renderBlockingJs > 10) findings.push({ cat: 'Performance', issue: `${html.renderBlockingJs} render-blocking scripts`, impact: 'Medium', desc: `${html.jsFiles} script tags total, ${html.renderBlockingJs} without defer/async. Slows the first paint a visitor sees.` });
  if (html.lazyLoadedImages === 0 && html.totalImages > 3) findings.push({ cat: 'Performance', issue: 'No lazy-loaded images', impact: 'Low', desc: `All ${html.totalImages} images load eagerly. On mobile this wastes bandwidth on images the visitor may never scroll to.` });
  ...
  // AI-crawler blocking (AEO) — high-value, very topical.
  const ai = aeo.aiCrawlers || {};
  if (ai.blockedMajor && ai.blockedMajor.length) {
    findings.push({ cat: 'AEO', issue: `Blocking AI crawlers: ${ai.blockedMajor.join(', ')}`, impact: 'High', desc: `robots.txt tells ${ai.blockedMajor.length} major AI crawler${ai.blockedMajor.length === 1 ? '' : 's'} not to read the site. However well the page is structured for AI, these engines are told to stay out — so you cannot appear in their answers. If this was not intentional, remove the Disallow rules for these user-agents.` });
  } else if (ai.blocksAllCrawlers) {
    findings.push({ cat: 'AEO', issue: 'robots.txt blocks all crawlers (Disallow: /)', impact: 'Critical', desc: 'robots.txt sets "User-agent: * / Disallow: /", asking every search and AI crawler to ignore the whole site. Unless this is a staging site, it is suppressing all organic and AI discoverability.' });
  }

  // Mixed content (Security).
  const mc = html.mixedContent || {};
  if (mc.count > 0) findings.push({ cat: 'Security', issue: `${mc.count} insecure (HTTP) resource${mc.count === 1 ? '' : 's'} on an HTTPS page`, impact: 'Medium', desc: `The page loads ${mc.count} subresource${mc.count === 1 ? '' : 's'} over plain http://${(mc.examples && mc.examples.length) ? ' (e.g. ' + mc.examples.slice(0, 2).join(', ') + ')' : ''}. Browsers may block them or drop the "fully secure" padlock.` });

  // Broken links (Best Practices) — only definitive failures.
  const lc = html.linkCheck || {};
  if (lc.checked && lc.broken && lc.broken.length) findings.push({ cat: 'Best Practices', issue: `${lc.broken.length} broken link${lc.broken.length === 1 ? '' : 's'} found in a ${lc.total}-link sample`, impact: 'Medium', desc: `Sampled internal links returned errors (e.g. ${lc.broken.slice(0, 2).map((b) => b.status + ' ' + b.url.replace(/^https?:\/\/[^/]+/, '')).join(', ')}). Broken links waste crawl budget and quietly erode trust.` });

  // Raw-vs-rendered diff (only when a rendered pass actually ran) — the AEO "hidden from AI crawlers" finding.
  if (html.rendering && html.rendering.rendered && html.rendering.diff && html.rendering.diff.onlyAfterRender.length) {
    const list = html.rendering.diff.onlyAfterRender;
    findings.push({ cat: 'AEO', issue: `${list.length} key signal${list.length === 1 ? '' : 's'} appear only after JavaScript runs`, impact: 'High', desc: `Present in the rendered page but missing from the raw server HTML: ${list.join(', ')}. Google executes JavaScript so it sees them — but most AI crawlers (GPTBot, ClaudeBot, PerplexityBot) do not, and see a near-empty shell. Server-side render or pre-render this content so AI answer engines can read and cite it.` });
  }

  // Client-rendered caveat — only when we could NOT render (e.g. ENABLE_RENDER off). Protects
  // credibility on SPA shells instead of confidently failing them.
  if (html.rendering && html.rendering.likelyClientRendered && !html.rendering.rendered) findings.push({ cat: 'Best Practices', issue: 'Content appears to be rendered by JavaScript', impact: 'Medium', desc: 'The server returned an almost-empty shell (a root <div> with little text) and the page is assembled in the browser. This scan read server HTML, so the Content, SEO, and AEO signals for this site are low-confidence and may understate the live page. It also matters for AI: many AI crawlers do not run JavaScript and may see the same empty shell.' });

  // Tag every finding with an effort-to-fix estimate (Config / Content / Dev).
  findings.forEach((f) => { f.effort = effortFor(f); });
...
// Strengths — the positive counterpart to findings. A report that only ever lists
// problems reads like a shakedown; surfacing what a site already does well makes the
// audit credible AND guarantees every dimension is represented (a clean dimension shows
// up here instead of silently producing nothing). One evidence-backed win per dimension.
function generateWins(data) {
  const { html, ssl, headers, observatory, pagespeed } = data;
  const seo = html.seo || {};
  const aeo = html.aeo || {};
  const wins = [];
  const add = (cat, headline, desc) => wins.push({ cat, headline, desc });

  // SEO
  const seoGood = [];
  if (seo.h1?.count >= 1) seoGood.push('a clear H1');
  if (seo.metaDescription?.exists) seoGood.push('a meta description');
  if (seo.title?.value && seo.title.length >= 15) seoGood.push('a descriptive title tag');
  if (seo.openGraph?.exists) seoGood.push('Open Graph tags');
  if (seoGood.length >= 3) add('SEO', 'On-page SEO basics are in place', `The homepage has ${seoGood.join(', ')} — the core signals search engines look for first.`);

  // AEO
  if (aeo.jsonLd?.exists && (aeo.organizationSchema || aeo.faqSchema)) add('AEO', 'AI engines can read your business identity', 'Structured data (JSON-LD) with an Organization/FAQ schema is present, so ChatGPT, Perplexity, and Gemini can extract and cite who you are.');
  if (aeo.aiCrawlers && aeo.aiCrawlers.checked && !aeo.aiCrawlers.any) add('AEO', 'AI crawlers are allowed in', 'robots.txt does not block the major AI crawlers (GPTBot, ClaudeBot, PerplexityBot, Google-Extended), so AI answer engines are free to read and cite your content.');

  // Security
  const sec = headers?.security || {};
  if (sec.headersSet >= 4) add('Security', `Strong security headers (${sec.headersSet}/${sec.headersTotal})`, 'Most recommended security headers (HSTS, CSP, X-Frame-Options, etc.) are configured — a sign of a well-maintained site.');
  if (observatory?.grade && ['A+', 'A', 'A-', 'B+', 'B'].includes(observatory.grade)) add('Security', `Security grade ${observatory.grade}`, `The site earns a ${observatory.grade} on the security assessment (score ${observatory.score}/100).`);
  if (ssl?.valid) add('Security', 'Valid SSL certificate', 'HTTPS is properly configured with a valid certificate — no browser warnings for visitors.');

  // Content
  if (seo.wordCount >= 300) add('Content', `Substantial homepage content (${seo.wordCount} words)`, 'There is enough copy for search engines and visitors to understand what you do.');
  if (seo.altText?.total > 0 && seo.altText?.pct >= 80) add('Content', `Images are well described (${seo.altText.pct}% alt text)`, 'Most images carry alt text — good for accessibility and image search.');

  // Performance
  const lhPerf = pagespeed?.scores?.performance;
  if (typeof lhPerf === 'number' && lhPerf >= 80) add('Performance', `Fast page (PageSpeed ${lhPerf}/100)`, 'Google Lighthouse rates mobile performance at 80 or above — visitors are not waiting on a slow page.');
  if (html.ttfb && html.ttfb < 400) add('Performance', `Quick server response (${html.ttfb}ms TTFB)`, 'The server starts responding in under 400ms — a snappy foundation for the rest of the page.');

  // Trust
  const trust = html.trust || {};
  if (trust.forms > 0) add('Trust', 'Visitors have a way to act', `The homepage has ${trust.forms} form${trust.forms === 1 ? '' : 's'} — interested visitors can reach out without hunting for contact details.`);

  // Accessibility
  const acc = html.accessibility || {};
  const lhA11y = pagespeed?.scores?.accessibility;
  if (typeof lhA11y === 'number' && lhA11y >= 90) add('Accessibility', `Strong accessibility score (${lhA11y}/100)`, 'Lighthouse finds few accessibility issues — good for inclusion and lower ADA risk.');
  else if (acc.landmarks > 0) add('Accessibility', 'Semantic structure for assistive tech', 'The page uses landmark regions (main/nav/header/footer), giving screen-reader users a map to navigate by.');

  // Best practices
  const bp = html.bestPractices || {};
  if (bp.https) add('Best Practices', 'Served securely over HTTPS', 'The page loads over HTTPS, so browsers show the secure padlock rather than a "Not secure" warning.');
  const lhBp = pagespeed?.scores?.['best-practices'];
  if (typeof lhBp === 'number' && lhBp >= 90) add('Best Practices', `Clean technical best practices (${lhBp}/100)`, 'No major console errors, deprecated APIs, or known-vulnerable libraries were detected.');

  // Internationalization
  const intl = html.i18n || {};
  if (intl.hreflang) add('Localization', 'Multi-language routing is set up', 'hreflang annotations are present, so search engines can serve the right language/region version to each visitor.');

  // Analytics
  const an = html.analytics || {};
  if (an.hasAny) add('Analytics', 'You can measure what works', `Analytics is installed${an.tools && an.tools.length ? ' (' + an.tools.slice(0, 3).join(', ') + ')' : ''} — you can see traffic and the impact of changes instead of guessing.`);
  if (an.hasAdPixel) add('Analytics', 'Conversion tracking is live', 'An advertising/conversion pixel is present, so paid campaigns can optimize toward real outcomes.');

  // Privacy
  const pv = html.privacy || {};
  if (pv.hasPrivacyPolicy) add('Privacy', 'Privacy policy is linked', 'A privacy policy is present — a baseline legal expectation under GDPR/CCPA and most ad platforms.');
  if (an.hasAny && pv.cookieConsent) add('Privacy', 'Consent banner protects you', 'Trackers load behind a cookie-consent mechanism — the compliant setup for EU/UK and California visitors.');
