...
async function fetchAndParseHtml(url) {
  ...
  // Heading hierarchy check (H1 -> H2 -> H3 should be sequential, no skipping)
  const allHeadings = [];
...
function calculateScores(data) {
  ...
  if (data.securityTxt?.exists) secScore += 5;
  ...
  if (seo.htmlLang) secScore += 5;
  if (seo.headingHierarchy) secScore += 5;
...
function generateFindings(data) {
  ...
  if (!seo.headingHierarchy) findings.push({ cat: 'SEO', issue: 'Heading hierarchy broken', impact: 'Medium', desc: 'Headings skip levels (e.g. H1 jumps to H3). Proper H1→H2→H3 nesting helps search engines understand content structure.' });
