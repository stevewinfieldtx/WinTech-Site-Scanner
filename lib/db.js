...
// The most recent full scan for this site BEFORE the given scan — powers the score-trend delta.
async function getPreviousScan(siteId, beforeScanId) {
  const result = await pool.query(
    `SELECT * FROM scans WHERE site_id = $1 AND id < $2 AND scan_type = 'full' ORDER BY id DESC LIMIT 1`,
    [siteId, beforeScanId]);
  return result.rows[0] || null;
}
